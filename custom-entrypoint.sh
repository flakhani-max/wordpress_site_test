...
# Run the official entrypoint logic (setup, config, etc.)
/usr/local/bin/docker-entrypoint.sh apache2-foreground & pid=$!

# Wait for WordPress HTTP to be up (optional, but recommended)
...
# Ensure siteurl and home use the correct port for development
...
# DO NOT CHANGE THIS URL FORMAT. Replace YOUR_KEY_HERE with your actual ACF Pro key.
ACF_PRO_KEY="NGIzMjdmZWQ3ZTdmNzM2MjRjZTBmOGM2ZTJkMTFkOTJiMTdmZjk4MmRjZWQyODAxY2Q1Mjli"
if ! wp plugin is-installed advanced-custom-fields-pro --allow-root; then
  wp plugin install "https://connect.advancedcustomfields.com/index.php?p=pro&a=download&k=${ACF_PRO_KEY}" --activate --allow-root || true
fi
